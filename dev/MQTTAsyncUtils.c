...
static MQTTAsync_queuedCommand *
MQTTAsync_restoreCommand(char *buffer, int buflen, int MQTTVersion, MQTTAsync_queuedCommand *);
...
static MQTTAsync_queuedCommand *
MQTTAsync_restoreCommand(char *buffer, int buflen, int MQTTVersion, MQTTAsync_queuedCommand *qcommand) {
    ...
    switch (command->type) {
        case SUBSCRIBE:
            ...
            if (command->details.sub.count > 0) {
                if ((command->details.sub.topics = (char **) malloc(sizeof(char *) * command->details.sub.count)) ==
                    NULL)
                    goto error_exit;
                ...
                if ((MQTTVersion >= MQTTVERSION_5)) {
                    if (command->details.sub.count > 1) {
                        command->details.sub.optlist = (MQTTSubscribe_options *) malloc(
                                sizeof(MQTTSubscribe_options) * command->details.sub.count);
        ...
        while (rc == 0 && i < nkeys) {
            ...
            if (strncmp(msgkeys[i], PERSISTENCE_COMMAND_KEY, strlen(PERSISTENCE_COMMAND_KEY)) != 0 &&
                ...
                if ((rc = c->persistence->pget(c->phandle, msgkeys[i], &buffer, &buflen)) == 0 &&
                    ...
                    int MQTTVersion = (strncmp(msgkeys[i], PERSISTENCE_V5_COMMAND_KEY,
                                               strlen(PERSISTENCE_V5_COMMAND_KEY)) == 0)
...
int MQTTAsync_addCommand(MQTTAsync_queuedCommand *command, int command_size) {
    ...
    if (command->command.type == CONNECT ||
        ...
        if (head) {
            ...
            ListInsert(MQTTAsync_commands, command, command_size,
                       MQTTAsync_commands->first); /* add to the head of the list */
    } else {
        ...
        if (command->client->c->persistence) {
            if (command->command.type == PUBLISH &&
                ...
                command->client->createOptions->persistQoS0 == 0 &&
                command->command.details.pub.qos == 0); /* don't persist QoS0 if that create option is set to 0 */
            else {
        ...
        if (command->command.type == PUBLISH) {
            ...
            if (command->client->createOptions &&
                (command->client->noBufferedMessages >= command->client->createOptions->maxBufferedMessages)) {
...
void MQTTAsync_checkDisconnect(MQTTAsync handle, MQTTAsync_command *command) {
    ...
    if (m->c->outboundMsgs->count == 0 ||
        MQTTTime_elapsed(command->start_time) >= (ELAPSED_TIME_TYPE) command->details.dis.timeout) {
    ...
    */
    ...
    while (ListNextElement(MQTTAsync_commands, &cur_command)) {
        ...
        if (cmd->command.type == CONNECT || cmd->command.type == DISCONNECT || (cmd->client->c->connected &&
                                                                                cmd->client->c->connect_state ==
                                                                                NOT_IN_PROGRESS &&
                                                                                MQTTAsync_Socket_noPendingWrites(
                                                                                        cmd->client->c->net.socket))) {
            if ((cmd->command.type == PUBLISH || cmd->command.type == SUBSCRIBE || cmd->command.type == UNSUBSCRIBE) &&
                cmd->client->c->outboundMsgs->count >= MAX_MSG_ID - 1) { ; /* no more message ids available */
    ...
    if (command) {
        ...
        if (command->client->c->persistence) {
            if (command->not_restored) {
                ...
                if ((rc = command->client->c->persistence->pget(command->client->c->phandle, command->key, &buffer,
                                                                &buflen)) == 0
                    && (command->client->c->afterRead == NULL ||
                        (rc = command->client->c->afterRead(command->client->c->afterRead_context, &buffer, &buflen)) ==
                        0)) {
                    int MQTTVersion = (strncmp(command->key, PERSISTENCE_V5_COMMAND_KEY,
                                               strlen(PERSISTENCE_V5_COMMAND_KEY)) == 0)
                                      ? MQTTVERSION_5 : MQTTVERSION_3_1_1;
                    free(command->key);
                    command->key = NULL;
    ...
    if (command->command.type == CONNECT) {
        if (command->client->c->connect_state != NOT_IN_PROGRESS || command->client->c->connected)
            ...
            Log(TRACE_PROTOCOL, -1, "Connecting to serverURI %s with MQTT version %d", serverURI,
                command->command.details.conn.MQTTVersion);

            rc = MQTTProtocol_connect(serverURI, command->client->c, command->client->websocket,
                                      command->command.details.conn.MQTTVersion, command->client->connectProps,
                                      command->client->willProps, 100);
        ...
        for (i = 0; i < command->command.details.unsub.count; i++)
            ListAppend(topics, command->command.details.unsub.topics[i],
                       strlen(command->command.details.unsub.topics[i]));
        ...
        rc = MQTTProtocol_startPublish(command->client->c, p, command->command.details.pub.qos,
                                       command->command.details.pub.retained, &msg);
    ...
    if (command->command.type == CONNECT && rc != SOCKET_ERROR && rc != MQTTASYNC_PERSISTENCE_ERROR) {
        ...
        if (command->command.type == CONNECT) {
            ...
            MQTTAsync_disconnect(command->client,
                                 &opts); /* not "internal" because we don't want to call connection lost */
...
static void MQTTAsync_checkTimeouts(void) {
    ...
    while (ListNextElement(MQTTAsync_handles, &current))        /* for each client */
    {
        ...
        if (m->c->connect_state != NOT_IN_PROGRESS &&
            MQTTTime_elapsed(m->connect.start_time) > (ELAPSED_TIME_TYPE) (m->connectTimeout * 1000)) {
        ...
        if (m->automaticReconnect && m->retrying) {
            if (m->reconnectNow ||
                MQTTTime_elapsed(m->lastConnectionFailedTime) > (ELAPSED_TIME_TYPE) (m->currentInterval * 1000)) {
                ...
                if (pack->header.bits.type == CONNACK) {
                    ...
                    if (rc == MQTTASYNC_SUCCESS) {
                        ...
                        if (m->c->MQTTVersion >= MQTTVERSION_5) {
                            if (MQTTProperties_hasProperty(&connack->properties, MQTTPROPERTY_CODE_RECEIVE_MAXIMUM)) {
                                int recv_max = MQTTProperties_getNumericValue(&connack->properties,
                    ...
                    while (ListNextElement(m->responses, &current)) {
                        ...
                        if (command->command.token == ((Suback *) pack)->msgId) {
                            ...
                            if (m->c->MQTTVersion >= MQTTVERSION_5) {
                                if (sub->qoss->count == 1 &&
                                    *(int *) (sub->qoss->first->content) >= MQTTREASONCODE_UNSPECIFIED_ERROR) {
                                    ...
                                    if (sub->qoss->count > 1) {
                                        ...
                                        enum MQTTReasonCodes *element = array = data.alt.sub.reasonCodes = malloc(
                                                sub->qoss->count * sizeof(enum MQTTReasonCodes));
                                        if (array)
                                    ...
                                    if (array)
                                       ...
                                       *(int *) (sub->qoss->first->content) == MQTT_BAD_SUBSCRIBE) {
                                if (command->command.onFailure) {
                                    MQTTAsync_failureData data;
                            ...
                            if (command->command.onSuccess || command->command.onSuccess5) {
                                ...
                                if (command->command.onSuccess) {
                                    ...
                                    if (unsub->reasonCodes->count > 1) {
                                        ...
                                        enum MQTTReasonCodes *element = array = data.alt.unsub.reasonCodes = malloc(
                                                unsub->reasonCodes->count * sizeof(enum MQTTReasonCodes));
                                        if (array)
...
static void MQTTAsync_stop(void) {
    ...
    if (sendThread_state != STOPPED || receiveThread_state != STOPPED) {
        ...
        if (conn_count == 0) {
            int count = 0;
            MQTTAsync_tostop = 1;
            while ((sendThread_state != STOPPED || receiveThread_state != STOPPED) && MQTTAsync_tostop != 0 &&
                   ++count < 100) {
                MQTTAsync_unlock_mutex(mqttasync_mutex);
...
static int MQTTAsync_connecting(MQTTAsyncs *m) {
    ...
    if (m->connect.details.conn.MQTTVersion == MQTTVERSION_DEFAULT) /* should not happen - #1007 */
    {
        ...
        m->connect.details.conn.MQTTVersion = (m->c->MQTTVersion == MQTTVERSION_DEFAULT) ? MQTTVERSION_3_1_1
                                                                                         : m->c->MQTTVersion;
    }
    ...
    else if (m->c->connect_state == WEBSOCKET_IN_PROGRESS) /* Websocket connect sent - wait for completion */
    {
        if ((rc = WebSocket_upgrade(&m->c->net)) == SOCKET_ERROR)
            ...
            if ((rc = MQTTPacket_send_connect(m->c, m->connect.details.conn.MQTTVersion, m->connectProps,
                                              m->willProps)) == SOCKET_ERROR)
                goto exit;
        }
    }

    exit:
    if ((rc != 0 && rc != TCPSOCKET_INTERRUPTED &&
         (m->c->connect_state != SSL_IN_PROGRESS && m->c->connect_state != WEBSOCKET_IN_PROGRESS)) || (rc == SSL_FATAL))
