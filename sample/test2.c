...
#define thread_return_type pthread_t
...
thread_return_type WINAPI test1_sendAndReceive(void *n) {
    ...
    for (i = 1; i <= iterations; ++i) {
        if (i % 10 == 0)
            MQTTClient_publish5(c, test_topic, test1_pubmsg.payloadlen, test1_pubmsg.payload,
...
//	opts.will = NULL;
    ...
    struct thread_parms parms0 = {c, 1, test_topic};
    ...
    struct thread_parms parms2 = {c, 1, test_topic};
    ...
    int wait_seconds = 90;
    while (((test1_arrivedcount < iterations * 3) || (test1_deliveryCompleted < iterations * 2)) &&
           (wait_seconds-- > 0)) {
#if defined(_WIN32)
        Sleep(1000);
#else
...
void test2_sendAndReceive(MQTTClient *c, int qos, char *test_topic) {
    ...
    for (i = 1; i <= iterations; ++i) {
        if (i % 10 == 0)
            MQTTClient_publish5(c, test_topic, test2_pubmsg.payloadlen, test2_pubmsg.payload,
...
int test2(struct Options options) {
    ...
    MQTTClient_create(&c, options.connection, "multi_threaded_sample");
