...
{
    List* newl = malloc(sizeof(List)); //todo use paho malloc?
    if (newl)
        ListZero_(newl);
    return newl;
}
...
{
    ListElement* newel = malloc(sizeof(ListElement)); //todo use paho malloc?
    if (newel)
        ListAppendNoMalloc_(aList, content, newel, size);
    return newel;
}
...
{
    return ListFindItem_(aList, content, NULL);
}
...
ListElement* ListFindItem_(List* aList, void* content, int(*callback)(void*, void*))
{
    ...
    else
    {
        ...
        /* find the content */
        while (ListNextElement_(aList, &current) != NULL)
        {
            if (callback == NULL)
            {
...
static int ListUnlink(List* aList, void* content, int(*callback)(void*, void*), int freeContent)
{
    ...
    if (!ListFindItem_(aList, content, callback))
